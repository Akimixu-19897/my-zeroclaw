...
mod tests {
    ...
    async fn deliver_announcement_accepts_wecom_channel() {
        ...
        let err = deliver_announcement(&config, "wecom", None, "chat:chat-alice", "hello")
            .await
            .unwrap_err();
        assert!(
            err.to_string().contains("connect WeCom websocket"),
            "expected proactive wecom delivery to attempt a real websocket send, got: {err}"
        );
    }
    ...
    async fn deliver_announcement_accepts_named_wecom_account() {
        ...
        let err = deliver_announcement(&config, "wecom", Some("ops"), "group:chat-ops", "hello")
            .await
            .unwrap_err();
        assert!(
            err.to_string().contains("connect WeCom websocket"),
            "expected named wecom account delivery to attempt a real websocket send, got: {err}"
        );
    }
    ...
    #[tokio::test]
    async fn deliver_announcement_rejects_wecom_user_target_for_proactive_send() {
        let tmp = TempDir::new().unwrap();
        let mut config = test_config(&tmp).await;
        config.channels_config.wecom = Some(WeComConfig {
            bot_id: "aib_test".into(),
