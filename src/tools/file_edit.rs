...
mod tests {
    ...
    #[tokio::test]
    async fn file_edit_expands_tilde_path_outside_workspace() {
        ...
        let outside_dir = home.join("zeroclaw_test_file_edit_tilde");
        let outside_file = outside_dir.join("edit.txt");

        let _ = tokio::fs::remove_dir_all(&workspace).await;
        let _ = tokio::fs::remove_dir_all(&outside_dir).await;
        tokio::fs::create_dir_all(&workspace).await.unwrap();
        tokio::fs::create_dir_all(&outside_dir).await.unwrap();
        tokio::fs::write(&outside_file, "hello old world")
            .await
            .unwrap();
        ...
        let result = tool
            .execute(json!({
                "path": "~/zeroclaw_test_file_edit_tilde/edit.txt",
                "old_string": "old",
                "new_string": "new"
            }))
            .await
