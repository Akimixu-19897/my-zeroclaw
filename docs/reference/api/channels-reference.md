...
- For cron delivery, use `channel = "wecom"`, optionally set `account = "<name>"`, and target `to = "chat:<chatid>"` or `to = "group:<chatid>"`. `user:<userid>` is only valid for replying inside an existing inbound message context, not for proactive cron delivery.
